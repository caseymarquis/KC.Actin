...
namespace KC.Actin.Analyzer
{
        ...
        private static void AnalyzeSymbol(SymbolAnalysisContext context)
        {
            switch (context.Symbol) {
                case INamedTypeSymbol named:
                    //Class definition:
                    if (named.ExtendsActor(context) && !named.GetAttributes().IncludeSingletonOrPeer()) { 
                ...
                case IFieldSymbol field:
                    ...
                    if (field.ContainingType.ExtendsActor(context) && field.Type.ExtendsActor(context) && !field.GetAttributes().IncludeSingletonOrPeer()) {
                            var diagnostic = Diagnostic.Create(Rule_ActorMemberNeedsAttribute, field.Locations[0], field.Name);
                            context.ReportDiagnostic(diagnostic);
                    }
                    break;
                case IPropertySymbol property:
                    if (property.ContainingType.ExtendsActor(context) && property.Type.ExtendsActor(context) && !property.GetAttributes().IncludeSingletonOrPeer()) {
    ...
    public static class AnalyzerExtensions {
        ...
        public static bool IncludeSingletonOrPeer(this ImmutableArray<AttributeData> attributes) {
